...
export default function ShopView({ userId, onCardsChanged }: ShopViewProps) {
  ...
  return (
    <div className="p-10 space-y-12 animate-in fade-in duration-500">
      ...
      {/* ── Baú Inicial Gratuito ── */}
      {starterOpened !== null && (
        <section className="space-y-4">
          <div className="flex items-center gap-3">
            <div className="w-1 h-6 bg-emerald-400 rounded-full" />
            <h2 className="text-sm font-black text-white/50 uppercase tracking-[0.4em]">Baú Gratuito</h2>
          </div>

          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className={`relative flex items-center gap-6 rounded-3xl border p-6 overflow-hidden
              ${starterOpened
                ? 'border-white/5 bg-white/[0.02] opacity-60'
                : 'border-emerald-500/40 bg-gradient-to-r from-emerald-900/30 via-emerald-800/10 to-transparent'
              }`}
            style={{ boxShadow: starterOpened ? 'none' : '0 0 40px rgba(16,185,129,0.2)' }}
          >
            <div className={`w-20 h-20 rounded-2xl flex items-center justify-center shrink-0
              ${starterOpened ? 'bg-white/5' : 'bg-emerald-500/20 border border-emerald-500/30'}`}
            >
              {starterOpened
                ? <Check className="w-10 h-10 text-white/20" />
                : <Gift className="w-10 h-10 text-emerald-400" style={{ filter: 'drop-shadow(0 0 12px rgba(16,185,129,0.6))' }} />
              }
            </div>

            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                <h3 className={`text-xl font-black uppercase tracking-tight ${starterOpened ? 'text-white/30' : 'text-white'}`}>
                  Baú Inicial Gratuito
                </h3>
                {!starterOpened && (
                  <span className="px-2 py-0.5 bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 text-[8px] font-black uppercase tracking-widest rounded-md">
                    GRÁTIS
                  </span>
                )}
              </div>
              <p className={`text-xs mb-3 ${starterOpened ? 'text-white/20' : 'text-white/50'}`}>
                {starterOpened
                  ? 'Já foi aberto — boas batalhas!'
                  : '5 cartas Neutras aleatórias para começar sua jornada. Apenas uma vez.'
                }
              </p>
              {!starterOpened && (
                <div className="flex flex-col gap-2">
                  <button
                    ...
                    className="px-6 py-2.5 bg-emerald-500 hover:bg-emerald-400 disabled:opacity-50 text-black font-black uppercase tracking-widest text-xs rounded-xl transition-all flex items-center gap-2 w-fit"
                  >
                    {openingStarter ? (
                      <><div className="w-3 h-3 border-2 border-black border-t-transparent rounded-full animate-spin" /> Abrindo...</>
                    ) : (
                      <><Gift className="w-3.5 h-3.5" /> Abrir Baú</>
                    )}
                  </button>
                  {starterError && (
                    <p className="text-[10px] text-red-400 font-bold max-w-xs">
                      ⚠ {starterError}
                    </p>
                  )}
                </div>
            ...
            <div className={`text-right shrink-0 ${starterOpened ? 'opacity-30' : ''}`}>
              <div className="text-2xl font-black text-emerald-400">5</div>
              <div className="text-[9px] font-black text-white/30 uppercase tracking-widest">cartas</div>
              <div className="text-[9px] font-black text-emerald-400/60 uppercase tracking-widest mt-1">somente neutras</div>
            </div>
          </motion.div>
        </section>
      )}
